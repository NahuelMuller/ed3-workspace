...
uint32_t		ADC_BUF_0[SAMPLES_BUFFER], ADC_BUF_1[SAMPLES_BUFFER],					// Buffers DAC y ADC
				...
				MEMORIA[BLOQUES][SAMPLES_BUFFER];
...
void init_Hardware(void){	// Inicializacion de hardware
	...
	for(bloques=0;bloques<BLOQUES;bloques++){
		for(samples_buffer=0;samples_buffer<SAMPLES_BUFFER;samples_buffer++){
			MEMORIA[bloques][samples_buffer]=0;
		}
	}
...
void create_Colas(void){
	...
	/*queue_grabo = xQueueCreate(1, sizeof(uint32_t));
	...
	while(1){
		...
		if( xSemaphoreTake(queue_intermedia1_ready,( TickType_t ) 1) == pdPASS ){			// REVISHAAR TIEMPO DE ESPERA
			for(jota = 0; jota < SAMPLES_BUFFER; jota++){
				...
				ka = ka + MEMORIA[current_bloque][jota];

				xQueueSendToBack(queue_intermedia2, &ka, (TickType_t) 1);
				//if(!xQueueSendToBack(queue_intermedia2, &ka, (TickType_t) 0)) printf("cola llena");

				xSemaphoreGive(queue_intermedia2_ready);			// Lo puse abajo del FOR que copia la queue

				if(grabo == 1){//estoy grabando
					MEMORIA[current_bloque][jota] = ka;//grabo en memoria
				}
			}
			//xSemaphoreGive(queue_intermedia2_ready);
			current_bloque++;
			if(current_bloque==BLOQUES){//agregar: en vez de BLOQUES, poner los bloques usados.
				current_bloque=0;
				if(grabo==1){
